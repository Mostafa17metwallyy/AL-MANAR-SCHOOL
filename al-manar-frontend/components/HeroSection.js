    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 pt-32 flex flex-col md:flex-row justify-between items-center relative z-10">
        ...
        <div className="md:w-1/2 mb-10 md:mb-0">
          <h2 className={`text-teal-600 font-bold text-base mb-3 ${isArabic ? "text-right" : "text-left"}`}>
            {t.welcome}
          </h2>

          <h1
            className={`text-3xl md:text-4xl font-bold text-gray-800 leading-snug mb-5 ${
          ...
          <p
            className={`text-sm md:text-base text-gray-700 max-w-md leading-relaxed mb-6 ${
              isArabic ? "text-right" : "text-left"
            }`}
          >
            {t.body}
          ...
          {/* CTA Buttons — RTL when Arabic */}
          <div
            className={`flex flex-wrap items-center gap-3 ${
              isArabic ? "flex-row-reverse justify-end" : "flex-row"
            }`}
          >
            {/* Announcements (opens new tab, same site) */}
            <a
              href="/announcements"
              target="_blank"
              ...
              className={`inline-flex items-center justify-center px-5 py-2.5 rounded-full bg-teal-600 text-white text-sm md:text-base font-semibold hover:bg-teal-700 transition ${
                isArabic ? "order-1" : "order-none"
              }`}
            >
              {t.announcements}
            </a>

            {/* Admission Form (same page anchor) */}
            <a
              href="#admission"
