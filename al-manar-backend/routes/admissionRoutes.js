...
router.post("/", async (req, res) => {
  try {
    ...
    if (!slot || slot.reservedBy) {
      return res
        .status(400)
        .json({ error: "Slot already reserved or not found." });
    }
...
router.get("/", async (req, res) => {
  try {
    console.log("✅ [GET] /api/admission called");
    const all = await Admission.find();
    console.log(`📦 Found ${all.length} admissions`);
    res.json(all);
  } catch (err) {
    console.error("❌ Error in GET /api/admission:", err);
    res.status(500).json({
